...
#define PS2_SEL        30                        // กำหนดขา Select  เป็นขา 18
#define PS2_CLK        25                        // กำหนดขา Clock   เป็นขา 19
...
void loop()
{
  ...
  if(ps2x.Button(PSB_CIRCLE))                    // ถ้าปุ่มวงกลมถูกกด
   ...
   else if(ps2x.Button(PSB_CROSS))                // ถ้าปุ่มกากบาทถูกกด
   {
    ...
    {
      wang();
    glcd(1, 1, "Triangle=%d",sv1);                      // แสดงข้อความว่า Triangle บนหน้าจอ
    }
   ...
   // delay(0.1);
    ...
    {
      ss();
      glcd(1, 1, "stop        ");                      // ลบข้อความที่แสดงอยู่ออก
//      test_sv();
